...
use {
    ...
    nexis_metrics::{datapoint_debug, datapoint_error},
    nexis_rayon_threadlimit::get_thread_count,
    nexis_runtime::hardened_unpack::{unpack_genesis_archive, MAX_GENESIS_ARCHIVE_UNPACKED_SIZE},
    nexis_sdk::{
    ...
    nexis_storage_proto::{StoredExtendedRewards, StoredTransactionStatusMeta},
    ...
    fn insert_data_shred(
        ...
        if slot_meta.is_full() {
            ...
            datapoint_info!(
                ...
                (
                    "total_time_ms",
                    nexis_sdk::timing::timestamp() - slot_meta.first_shred_timestamp,
                    i64
                ),
...
// Chaining based on latest discussion here: https://github.com/nexis-netork/nexis/pull/2253
fn handle_chaining(
    db: &Database,
    write_batch: &mut WriteBatch,
...
pub fn create_new_ledger(
    ...
    let version = nexis_sdk::shred_version::version_from_hash(&last_hash);
...
pub fn create_new_ledger_from_name_auto_delete(
    ...
    let _root_hash =
        nexis_sdk::genesis_config::evm_genesis::generate_evm_state_json(&evm_state_json).unwrap();
...
pub mod tests {
    use {
        ...
        nexis_entry::entry::{next_entry, next_entry_mut},
        nexis_runtime::bank::{Bank, RewardType},
        nexis_sdk::{
            hash::{self, hash, Hash},
        ...
        nexis_transaction_status::{InnerInstructions, Reward, Rewards, TransactionTokenBalance},
    ...
    */
    ...
    fn test_persist_transaction_status() {
        ...
        // insert value
        let status = TransactionStatusMeta {
            status: nexis_sdk::transaction::Result::<()>::Err(TransactionError::AccountNotFound),
            fee: 5u64,
        ...
        // insert value
        let status = TransactionStatusMeta {
            status: nexis_sdk::transaction::Result::<()>::Ok(()),
            fee: 9u64,
            pre_balances: pre_balances_vec.clone(),
            post_balances: post_balances_vec.clone(),
            inner_instructions: Some(inner_instructions_vec.clone()),
    ...
    fn do_test_lowest_cleanup_slot_and_special_cfs(
        ...
        let address0 = nexis_sdk::pubkey::new_rand();
        let address1 = nexis_sdk::pubkey::new_rand();
        blockstore
            .write_transaction_status(
                lowest_cleanup_slot,
                signature1,
    ...
    fn test_map_transactions_to_statuses() {
        ...
        for x in 0..4 {
            ...
            let status = TransactionStatusMeta {
                status: nexis_sdk::transaction::Result::<()>::Err(
    ...
    fn test_rewards_protobuf_backward_compatability() {
        ...
        let rewards: Rewards = (0..100)
            .map(|i| Reward {
                pubkey: nexis_sdk::pubkey::new_rand().to_string(),
                lamports: 42 + i,
    ...
    fn make_large_tx_entry(num_txs: usize) -> Entry {
        let txs: Vec<_> = (0..num_txs)
            ...
            .map(|_| {
                ...
                nexis_sdk::system_transaction::transfer(&keypair0, &to, 1, Hash::default())
            })
            .collect();
