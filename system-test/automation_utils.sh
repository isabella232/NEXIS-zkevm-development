...
function analyze_packet_loss {
  (
    ...
    for ip in "${validatorIpList[@]}"; do
      "${REPO_ROOT}"/net/scp.sh ip_address_map.txt nexis@"$ip":~/nexis/
    done
    ...
    for ip in "${validatorIpList[@]}"; do
      ...
      "${REPO_ROOT}"/net/ssh.sh nexis@"$ip" 'PATH=$PATH:~/.cargo/bin/ ~/nexis/scripts/iftop-postprocess.sh ~/nexis/iftop.log temp.log ~nexis/nexis/ip_address_map.txt' > "$iftop_log"
      upload-ci-artifact "$iftop_log"
    done

    execution_step "Analyzing Packet Loss"
    "${REPO_ROOT}"/nexis-release/bin/nexis-log-analyzer analyze -f ./iftop-logs/ | sort -k 2 -g
  )
}
...
function wait_for_max_stake {
  ...
  ssh "${sshOptions[@]}" "${validatorIpList[0]}" "RUST_LOG=info \$HOME/.cargo/bin/nexiswait-for-max-stake $max_stake --url http://127.0.0.1:8899"
}
...
function get_slot {
  source "${REPO_ROOT}"/net/common.sh
  loadConfigFile
  ssh "${sshOptions[@]}" "${validatorIpList[0]}" '$HOME/.cargo/bin/nexis--url http://127.0.0.1:8899 slot'
}
...
function get_active_stake {
  ...
  ssh "${sshOptions[@]}" "${validatorIpList[0]}" \
    '$HOME/.cargo/bin/nexis--url http://127.0.0.1:8899 validators --output=json | grep -o "totalActiveStake\": [0-9]*" | cut -d: -f2'
}
...
function get_current_stake {
  ...
  loadConfigFile
  ssh "${sshOptions[@]}" "${validatorIpList[0]}" \
    '$HOME/.cargo/bin/nexis--url http://127.0.0.1:8899 validators --output=json | grep -o "totalCurrentStake\": [0-9]*" | cut -d: -f2'
}
...
function upload_results_to_slack() {
  ...
  COMMIT_URL="https://github.com/nexis-netork/nexis/commit/${COMMIT}"

  if [[ -n $BUILDKITE_BUILD_URL ]] ; then
    BUILD_BUTTON_TEXT="Build Kite Job"
  else
    BUILD_BUTTON_TEXT="Build URL not defined"
    BUILDKITE_BUILD_URL="https://buildkite.com/nexis-labs/"
  fi

  GRAFANA_URL="https://metrics.nexis.network:3000/d/monitor-${CHANNEL:-edge}/cluster-telemetry-${CHANNEL:-edge}?var-testnet=${TESTNET_TAG:-testnet-automation}&from=${TESTNET_START_UNIX_MSECS:-0}&to=${TESTNET_FINISH_UNIX_MSECS:-0}"
