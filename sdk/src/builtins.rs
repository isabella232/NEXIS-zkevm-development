    ...
    ($name:ident, $id:path, $entrypoint:expr) => {
        ...
        macro_rules! $name {
            () => {
                ...
                // see https://github.com/nexis-netork/nexis/issues/10933.
                // For now, we need to use `::nexis_sdk`
                //
                // `respan!` respans the path `$crate::id`, which we then call (hence the extra
                // parens)
                (
                    stringify!($name).to_string(),
                    ::nexis_sdk::respan!($crate::$id, $name)(),
                    $entrypoint,
                )
            };
        }
...
///
/// # use item_wrapper::id;
/// assert_eq!(id(), nexis_sdk::system_program::ID);
/// ```
#[macro_export]
