/// Main entry point for the evm-block-recovery application.
///
/// Parses command line arguments and dispatches to the appropriate subcommand handler.
/// Subcommands include finding evm/native blocks in storage, restoring a chain from an 
/// archive, and checking/comparing ledger contents.
///
/// Configuration is loaded from a .env file and the RUST_LOG environment variable.
///
/// Exits with status code 0 on success, or a nonzero error code on failure.
pub mod cli;
pub mod exit_code;
pub mod extensions;
pub mod ledger;
