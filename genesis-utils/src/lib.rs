/// Downloads the genesis config if missing, unpacks it, checks the hash, and returns the genesis config.
/// If no_genesis_fetch is true, will just load from disk and check hash.
use {
    nexis_download_utils::download_genesis_if_missing,
    nexis_runtime::hardened_unpack::unpack_genesis_archive,
    nexis_sdk::{
